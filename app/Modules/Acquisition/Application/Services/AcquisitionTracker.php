...
final class AcquisitionTracker
{
    ...
    public function capture(Request $request, ?AcquisitionCampaign $campaign = null): AcquisitionVisit
    {
        ...
        $visit = AcquisitionVisit::query()->create([
            ...
            'user_agent' => $this->header($request, 'user-agent', 500),
        ]);
    ...
    public function currentVisit(Request $request): ?AcquisitionVisit
    {
        ...
        return AcquisitionVisit::query()->with('campaign.venue.location.address')->find((int) $id);
    }
    ...
    public function updateLocation(
        ...
        if ($status !== 'granted') {
            $normalized = in_array($status, ['denied', 'unavailable'], true) ? $status : 'unavailable';
            $visit->forceFill([
