...
final class ShowVenueHandler
{
        ...
        $occupiedSlots = $venue->bookings()
            ...
            ->map(function ($booking) use ($timezone): array {
                ...
                $eventTypeLabel = $event !== null
                    ...
                    : 'Бронирование';
                $statusLabel = $this->occupiedSlotStatusLabel($booking->status);
                $isPublicEvent = $event !== null
                    && $event->status === EventStatusEnum::PUBLISHED
                    && $event->visibility === EventVisibilityEnum::PUBLIC;
                $timeLabel = $startsAt->format('d.m.Y H:i').'–'.$endsAt->format('H:i');

                return [
                    'label' => $isPublicEvent
                        ? $timeLabel
                        : $timeLabel.' · '.$eventTypeLabel.' · '.$statusLabel,
                    ...
                    'eventTitle' => null,
                    'eventTypeLabel' => $isPublicEvent
                        ? $eventTypeLabel.' · '.$statusLabel
                        : null,
    ...
    private function occupiedSlotStatusLabel(VenueBookingStatusEnum $status): string
    {
        return match ($status) {
            VenueBookingStatusEnum::CONFIRMED => '✅ '.$status->label(),
            VenueBookingStatusEnum::PENDING => '🕒 '.$status->label(),
            VenueBookingStatusEnum::HELD => '⏳ '.$status->label(),
