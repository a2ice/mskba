...
final class StandaloneGameInitialSelectionService
{
    ...
    public function apply(Game $game, Actor $actor, ?int $teamAId, ?int $teamBId): Game
    {
        ...
        return DB::transaction(function () use ($game, $actor, $teamAId, $teamBId, $selectedIds): Game {
            Event::query()->whereKey($game->event_id)->lockForUpdate()->firstOrFail();
            ...
            foreach ($selectedIds as $teamId) {
                ...
                $this->admissions->invite($lockedGame, $actor, $team);
            }

            if ($teamAId !== null
                && $teamBId !== null
                && $acceptedTeamIds->unique()->count() === 2) {
                return $this->formation->confirmTeams($lockedGame, $actor, (int) $teamAId, (int) $teamBId);
            }

            return $lockedGame->fresh(['admissions', 'sides', 'rosterEntries']);
        }, 3);
    }
