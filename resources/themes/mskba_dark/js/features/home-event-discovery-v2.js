...
function initHomeEventDiscovery(flow) {
    ...
    function locationFilters() {
        ...
        const location = flow.querySelector('.home-event-location--conditional');
        ...
        const filters = {
            street: location.dataset.homeLocationStreet || '',
            venue_id: venueId,
        };
        const filterMode = location.dataset.homeLocationFilterMode || 'none';

        if (filterMode === 'radius') {
            filters.latitude = location.dataset.homeLocationLatitude || '';
            filters.longitude = location.dataset.homeLocationLongitude || '';
            filters.radius_km = location.dataset.homeLocationRadiusKm || '';
        } else if (filterMode === 'administrative') {
            filters.city_id = location.dataset.homeLocationCityId || '';
            filters.district_id = location.dataset.homeLocationDistrictId || '';
        }
    ...
    $(document).on('modal:opened.homeEventDiscoveryV2', function (_event, modal) {
        if (modal.find('[data-home-flow="event"]').get(0) === flow && inResults) {
            leaveResults();
        }
    });

    $(document).on('click.homeEventDiscoveryV2', '[data-home-flow="event"] [data-home-flow-tab]', function () {
        if (inResults) {
            leaveResults();
        }
    });
