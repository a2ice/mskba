@php
    ...
    $statisticsConfirmed = $event->gameDetail->statistics_status === GameStatisticsStatusEnum::CONFIRMED;
    $isCancelled = $event->status === EventStatusEnum::CANCELLED;
    $isCompleted = $event->status === EventStatusEnum::COMPLETED || $statisticsConfirmed;
    $effectiveStartsAt = $event->gameDetail->is_time_scheduled
        ? $event->starts_at
        : ($event->parentEvent?->starts_at ?? $event->starts_at);
    $effectiveEndsAt = $event->gameDetail->is_time_scheduled
        ? $event->ends_at
        : ($event->parentEvent?->ends_at ?? $event->ends_at);
    ...
    $parentCandidates = $event->parentEvent?->participants
    ...
    <section class="game-control first-screen" data-game-control>
        <div class="inner game-control__inner">
            ...
            @if($event->parentEvent)
                <a class="game-control__back" href="{{ route('events.show', $event->parentEvent->routeIdentifier()) }}"><i class="ti ti-arrow-left"></i>Назад к мероприятию</a>
            @endif

            <header class="game-control__header">
                <div><span class="eyebrow">{{ $event->parentEvent ? 'Мини-игра' : 'Игра' }}</span><h1>{{ $event->title }}</h1>@if($event->parentEvent)<p>В рамках «<a href="{{ route('events.show', $event->parentEvent->routeIdentifier()) }}">{{ $event->parentEvent->title }}</a>»</p>@endif</div>
                <div class="game-control__chips"><span>{{ $event->gameDetail->formatLabel() }}</span><span>{{ $event->gameDetail->scoring_type->label() }}</span><span class="{{ $gameState[1] }}"><i class="ti ti-point-filled"></i>{{ $gameState[0] }}</span><span><i class="ti ti-users"></i>Команды из участников</span></div>
                <div class="game-control__meta"><span><i class="ti ti-map-pin"></i>{{ $event->venue->name }}</span><span><i class="ti ti-clock"></i>{{ $event->gameDetail->is_time_scheduled ? $event->starts_at->format('H:i').'–'.$event->ends_at->format('H:i') : 'Время на игру не задано' }}</span></div>
            </header>

            <section class="game-scoreboard" data-game-scoreboard data-score-url="{{ route('events.game.score', $event->routeIdentifier()) }}" data-image-upload-surface>
                ...
                @if($canUpdate && !$statisticsConfirmed && !$isCancelled)<details class="game-control-editor"><summary class="btn btn--secondary btn--sm"><i class="ti ti-pencil"></i>Редактировать</summary><form method="POST" action="{{ route('events.game.update', $event->routeIdentifier()) }}" class="row g-3">@csrf @method('PUT')<div class="col-12"><label class="form-label">Название</label><input class="form-control" name="title" value="{{ $event->title }}" required></div><input type="hidden" name="has_scheduled_time" value="{{ (int) $event->gameDetail->is_time_scheduled }}"><input type="hidden" name="starts_at" value="{{ $event->gameDetail->is_time_scheduled ? $event->starts_at->format('H:i') : '' }}"><input type="hidden" name="ends_at" value="{{ $event->gameDetail->is_time_scheduled ? $event->ends_at->format('H:i') : '' }}"><input type="hidden" name="side_a_size" value="{{ $event->gameDetail->side_a_size }}"><input type="hidden" name="side_b_size" value="{{ $event->gameDetail->side_b_size }}"><input type="hidden" name="scoring_type" value="{{ $event->gameDetail->scoring_type->value }}"><div class="col-md-6"><label class="form-label">Команда A</label><input class="form-control" name="side_a_name" value="{{ $sideA?->display_name }}"></div><div class="col-md-6"><label class="form-label">Команда B</label><input class="form-control" name="side_b_name" value="{{ $sideB?->display_name }}"></div><div class="col-12"><button class="btn btn--primary btn--sm">Сохранить</button></div></form></details>@endif
            </section>

            <form method="POST" action="{{ route('events.game.statistics', $event->routeIdentifier()) }}" data-game-live-statistics data-scoring-type="{{ $event->gameDetail->scoring_type->value }}" data-image-upload-surface>
                        ...
                        <div class="game-team-panel__players">
                            @forelse($roster->get($side?->id, collect()) as $entry)
                                ...
                                <article class="game-live-player" data-game-player="{{ $entry->user_id }}" data-game-player-side="{{ $slot }}">
                                    ...
                                    <strong class="game-live-player__points"><span data-game-player-points>{{ $stat?->points($event->gameDetail->scoring_type) ?? 0 }}</span> очк.</strong>
