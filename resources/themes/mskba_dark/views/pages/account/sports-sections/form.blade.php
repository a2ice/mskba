...
<form method="POST" action="{{ $editing ? route('account.sports-sections.update', $section) : route('account.sports-sections.store') }}" class="sports-section-form" data-venue-court-group>@csrf @if($editing)@method('PUT')@endif
    ...
    <label class="form-field"><span>Игровое направление</span><select class="form-select" name="game_format">@foreach($formats as $item)<option value="{{ $item->value }}" @selected(old('game_format', $section?->game_format?->value ?? 'basketball') === $item->value)>{{ $item->label() }}</option>@endforeach</select></label>
