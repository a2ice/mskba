...
@forelse($sections as $section)<article><div><span class="text-accent">{{ $section->status->label() }}</span><h2>{{ $section->name }}</h2><p>{{ $section->game_format->label() }} · {{ $section->training_mode->label() }}</p></div><div><a class="btn btn--secondary btn--sm" href="{{ route('account.sports-sections.edit', $section) }}">Управлять</a>@if($applicationSectionIds->contains($section->id))<a class="btn btn--secondary btn--sm" href="{{ route('account.sports-sections.applications', $section) }}">Заявки и набор</a>@endif @if($teamRelationSectionIds->contains($section->id))<a class="btn btn--secondary btn--sm" href="{{ route('account.sports-sections.teams', $section) }}">Команды</a>@endif @if($section->status->value === 'active')<a class="btn btn--secondary btn--sm" href="{{ route('sports-sections.show', $section) }}">Открыть</a>@endif</div></article>
