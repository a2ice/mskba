...
@section('section-content')
    ...
    <section class="section-card" id="table"><h2>Турнирная таблица</h2><p class="text-muted mb-2">Учитываются только подтверждённые результаты. Победа — 2 очка, ничья — 1, поражение — 0.</p><div class="table-responsive"><table class="table"><thead><tr><th><span title="Текущая позиция команды в турнирной таблице" data-tooltip-visual tabindex="0">#</span></th><th>Команда</th><th><span title="Сыграно игр" data-tooltip-visual tabindex="0">И</span></th><th><span title="Победы" data-tooltip-visual tabindex="0">В</span></th><th><span title="Ничьи" data-tooltip-visual tabindex="0">Н</span></th><th><span title="Поражения" data-tooltip-visual tabindex="0">П</span></th><th><span title="Забитые и пропущенные мячи" data-tooltip-visual tabindex="0">Мячи</span></th><th><span title="Разница забитых и пропущенных мячей" data-tooltip-visual tabindex="0">Разн.</span></th><th><span title="Турнирные очки" data-tooltip-visual tabindex="0">Очки</span></th></tr></thead><tbody>@foreach($standings as $row)<tr><td>{{ $row['position'] }}</td><td>{{ $row['name'] }}</td><td>{{ $row['played'] }}</td><td>{{ $row['wins'] }}</td><td>{{ $row['draws'] }}</td><td>{{ $row['losses'] }}</td><td>{{ $row['scored'] }}:{{ $row['conceded'] }}</td><td>{{ $row['difference'] > 0 ? '+' : '' }}{{ $row['difference'] }}</td><td><strong>{{ $row['points'] }}</strong></td></tr>@endforeach</tbody></table></div></section>
@endsection