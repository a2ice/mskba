    ...
    <div class="sports-section-heading-actions">
        ...
        @elseif($section->accepts_trainee_requests)
            @auth
                ...
                @endif
            @else
                <button class="btn btn--primary btn--sm" type="button" data-handler="modal" data-modal-action="open" data-modal-target="auth-entry-classic" data-auth-redirect-url="{{ route('sports-sections.show', $section, false) }}">Записаться</button>
            @endauth
        @else
