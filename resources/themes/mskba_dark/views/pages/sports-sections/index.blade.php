...
<section class="sports-sections first-screen"><div class="inner">
    ...
    <form method="GET" action="{{ route('sports-sections.index') }}" class="sports-sections__filters">
        ...
        <label class="form-field"><span>Игровое направление</span><select class="form-select" name="game_format"><option value="">Любое</option>@foreach($formats as $item)<option value="{{ $item->value }}" @selected(request('game_format') === $item->value)>{{ $item->label() }}</option>@endforeach</select></label>
        ...
        <label class="form-field"><span>Заявки</span><select class="form-select" name="accepts_requests"><option value="">Любые</option><option value="1" @selected(request()->boolean('accepts_requests'))>Принимает заявки</option></select></label>
        <label class="form-field"><span>Набор</span><select class="form-select" name="recruiting"><option value="">Любой</option><option value="1" @selected(request()->boolean('recruiting'))>Идёт набор</option></select></label>
        <div class="sports-sections__filter-actions"><button class="btn btn--primary" type="submit">Найти</button>@if(request()->hasAny(['q','training_mode','game_format','pricing_type','venue_id','accepts_requests','recruiting']))<a class="btn btn--secondary" href="{{ route('sports-sections.index') }}">Сбросить</a>@endif</div>
    </form>
            ...
            <article class="sports-section-card">
                ...
                <div>
                    @if($section->is_recruiting || $section->accepts_trainee_requests)
                        <div class="sports-section-card__badges">
                    ...
                    @endif
                    <span class="text-muted">{{ $section->game_format->label() }} · {{ $section->training_mode->label() }}</span><h2><a href="{{ route('sports-sections.show', $section) }}">{{ $section->name }}</a></h2>
