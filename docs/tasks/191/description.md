...
Venue-backed campaign attribution и добровольная геопроверка остаются доступны и для авторизованного пользователя; отдельная venue-context карточка на landing больше не требуется.
...
Геолокация не может и не должна запрашиваться скрытно. Актуальный flow:

- если permission уже выдан браузеру — можно выполнить проверку автоматически;
- если permission ещё не запрашивался, геопроверка стартует из явного пользовательского жеста на основном onboarding CTA («Присоединиться» / «У меня уже есть аккаунт»), без отдельной промежуточной карточки;
- переход к следующему шагу wizard не зависит от результата геопроверки;
- отказ/ошибка GPS никогда не блокируют регистрацию или вход;
