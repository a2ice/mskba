...
Task 177 добавляет организационную связь с постоянными Team как M:N через
`sports_section_team`. Ни SportsSection, ни Team не становятся владельцем другой
сущности. Одна школа/секция может объединять несколько команд, а одна постоянная
команда может быть связана с несколькими секциями. Relation не синхронизирует
составы, ACL, площадки или расписание. Link требует управления обеими сущностями,
unlink не удаляет ни Team, ни SportsSection. TrainingSession/Event snapshots не
зависят от этой relation и не меняются задним числом.
...
Task 177 добавляет отдельный account-экран управления Team relation. На нём видны
уже связанные команды и только постоянные команды, которыми пользователь вправе
управлять. Relation хранится отдельно от настроек SportsSection и не используется
как источник прав доступа.
...
Task 177 не выводит права из relation: link требует `section.manage` и
`team.settings.edit`, а unlink требует `section.manage`. Это позволяет владельцу
секции безопасно отозвать связь, не получая при этом никаких прав на Team.
...
Task 177 добавляет проверки M:N relation, permanent-Team gate, link/unlink,
независимости ACL и IDOR account-экрана.
...
14.09.2026 Task 177 зафиксировал M:N связь SportsSection ↔ постоянная Team через `sports_section_team`; relation остаётся организационной и не объединяет ACL или snapshots двух контуров.
