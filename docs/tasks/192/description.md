...
- Свёрнутые окна объединены в общий многовкладочный tray: несколько popup сохраняются одновременно, вкладки сжимаются до 100 px и затем прокручиваются горизонтально.
- URL расширен параметром `modal_tray=<id1,id2,...>`: `modal` хранит развёрнутое окно, tray — свёрнутые; legacy `modal_state=minimized` остаётся читаемым.
- На мобильном экране tray уменьшен и через `--modal-tray-height` поднимает primary bar и нижний отступ страницы; desktop tray резервирует место под контент.
- После post-push аудита исправлены два edge case: `Escape` больше не закрывает скрытую вкладку tray вместо активного окна, а focus при ручном сворачивании переносится на соответствующую кнопку восстановления в tray.
