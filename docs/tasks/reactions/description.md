...
После 101 нужно отдельно проверить случай, когда TelegramAccount физически остаётся на alias: web/Telegram должны разрешаться в одного canonical actor. Если у identity несколько исторических TelegramAccount, схлопывание external actor должно учитывать всю identity group, а не только один account.
...
- временный reactions CI workflow удалить перед Ready/merge.
