...
`UpdateEventHandler` изменяет только поля, не влияющие на бронь: название, тип, описание, видимость и лимит участников. Он блокирует строку `events`, повторно проверяет право через `EventManagementAccess` и сравнивает новый лимит с числом подтверждённых участников. `JoinEventHandler` использует ту же блокировку строки события, поэтому конкурентные update/join не могут оба принять устаревшее значение вместимости. Перенос площадки или времени должен использовать отдельный обработчик с порядком блокировок `venue -> event -> booking`.
