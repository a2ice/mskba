...
- `Event` — мероприятие с типом `game|training|game_training|tournament`, организатором-actor, площадкой, локализованным временем, видимостью и лимитом участников;
...
`CreateEventRequest` требует старт не раньше округлённой минуты `now + 30 минут` и длительность не менее часа. Пустой `ends_at` нормализуется в `starts_at + 1 час`; `CreateEventHandler` повторяет этот fallback для вызовов вне HTTP. Форма лишь помогает начальными значениями и динамической синхронизацией, но бизнес-ограничения обеспечиваются сервером.
