...
Разработка ведется через задачи из `tasks.md`. Для каждой задачи создается папка `docs/tasks/{NNN}` с описанием, планом, статусом и итогами.
...
- После merge задача отмечается выполненной в `tasks.md` уже из `main`.
