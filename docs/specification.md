...
Создание площадки запрещает дубли по нормализованному alias, вычисленному из названия, только если совпадение найдено среди подтвержденных площадок. Старое поведение с автоматическим подбором суффиксов `-2`, `-3` не используется: для неподтвержденных дублей сохраняется одинаковый alias, а не суффиксированный вариант. Проверка не зависит от регистра исходного названия.

Создание площадки также проверяет дубли адреса среди подтвержденных площадок. Сравниваются нормализованные адресные строки из `venues.raw_address` и `addresses.full_address`; для structured-only адресов дополнительно сравниваются `city`, `street` и `building`.

Неподтвержденные площадки могут совпадать по названию и/или адресу. После создания площадки dispatchится `FindVenueDuplicatesJob`, который сравнивает запись с другими неподтвержденными площадками и создает pending-кандидаты в таблице `venue_duplicates`. Кандидат хранит пару `venue_id`/`duplicate_venue_id`, тип совпадения `matched_by`, статус `pending|merged|rejected`, score, metadata и поля resolution.

Повторное создание одинаковой неподтвержденной или заблокированной заявки тем же автором запрещено. Для авторизованного пользователя авторство сравнивается по `creatorActor.user_id`, для гостя - по fingerprint. Поэтому разные пользователи могут создать одинаковые неподтвержденные записи, но один пользователь не может создать две свои версии одной и той же площадки.
...
- `venue_moderation_requests` - заявка на рассмотрение площадки со статусом `pending|approved|rejected|blocked`;
- `venue_moderation_messages` - входящие сообщения пользователя и исходящие ответы модератора по заявке.

Владелец площадки открывает `/venues/{alias}/edit`, редактирует минимальные данные и отправляет заявку через `POST /venues/{alias}/moderation`. Пока есть pending-заявка, повторная отправка запрещена. Администратор рассматривает pending-заявки в `/admin/venues`: подтверждение переводит площадку в `confirmed`, отклонение сохраняет outgoing-сообщение, блокировка переводит площадку в `blocked` и заполняет `status_info`.

Каноническая точка входа находится на `/venues/create`, отправка формы идет через `POST /venues`. Личный кабинет не имеет отдельного create/store маршрута площадок и ведет пользователя на этот же публичный URL. GET-страница `/venues/create` и POST-сценарий доступны гостям, неподтвержденным и подтвержденным пользователям, кроме заблокированных аккаунтов. Происхождение заявки фиксируется через `Actor`: для гостя используется fingerprint, для авторизованного пользователя - user actor.
