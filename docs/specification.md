...
Канонический идентификатор маршрута площадки имеет вид `{id}-{alias}`, например `/venues/42-shkola-1794`: ID однозначно определяет запись, а alias сохраняет читаемость URL. Все новые ссылки используют этот формат. Alias-only URL временно остаются fallback-маршрутами для старых ссылок.
