...
Создание площадки запрещает дубли по нормализованному alias, вычисленному из названия. Старое поведение с автоматическим подбором суффиксов `-2`, `-3` не используется: если alias уже занят, пользователь должен получить validation error. Проверка не зависит от регистра исходного названия.

Создание площадки также проверяет дубли адреса. Сравниваются нормализованные адресные строки из `venues.raw_address` и `addresses.full_address`; для structured-only адресов дополнительно сравниваются `city`, `street` и `building`.
