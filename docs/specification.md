...
- `VenueStatusEnum` - жизненный статус записи: `unconfirmed`, `confirmed`, `duplicate`, `blocked`, `removed`;
- `VenueTypeEnum` - тип площадки.

`Venue` имеет relation `creatorActor()` к `Actor` по `created_by_actor_id`, relation `location()` к `Location` по `location_id` и relation `canonicalVenue()` к канонической площадке по `canonical_venue_id`.
...
Повторное создание одинаковой неподтвержденной/duplicate-заявки тем же автором запрещено. Для авторизованного пользователя авторство сравнивается по `creatorActor.user_id`, для гостя - по fingerprint. Поэтому разные пользователи могут отправить одинаковую неподтвержденную площадку на модерацию, но один пользователь не может создать две свои версии одной и той же площадки.
