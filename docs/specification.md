...
Endpoint `GET /venues/search` возвращает JSON-выдачу для Telegram-поиска и принимает необязательные параметры `query`, `type` и `metro_station_id`. `SearchVenuesHandler` начинает работу с результатом `ListVenuesHandler`, поэтому фильтрация не обходит общие правила видимости площадок; затем `query` ищется по названию, описанию, адресу, `VenueTag::name` и `MetroStation::name`, после чего применяются расширенные фильтры типа и точной станции метро и ограничение размера выдачи.
