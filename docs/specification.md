...
- `alias`;
- `type`;
- `is_free` - свободный доступ без оплаты и ручного подтверждения бронирования, `true` по умолчанию;
- `status`;
- `status_info`;
...
Endpoint `GET /venues/search` возвращает JSON-выдачу для Telegram-поиска и принимает необязательные параметры `query`, `type`, `metro_station_id` и `is_free`. `SearchVenuesHandler` начинает работу с результатом `ListVenuesHandler`, поэтому фильтрация не обходит общие правила видимости площадок; затем `query` ищется по названию, описанию, адресу, `VenueTag::name` и `MetroStation::name`, после чего применяются расширенные фильтры условий доступа, типа и точной станции метро и ограничение размера выдачи.

Будущий инвариант бронирования: если `venues.is_free = true`, создание брони свободного слота не должно переходить в ожидание ручного подтверждения или оплаты; достаточно атомарно убедиться в отсутствии пересекающейся активной брони. Проверку пересечений необходимо выполнять внутри транзакции с блокировкой или ограничением уровня БД, иначе два параллельных запроса смогут занять один интервал. Поля оплаты и ручного подтверждения при появлении независимых сценариев следует отделить от `is_free`, не расширяя неоднозначно смысл одного boolean.
