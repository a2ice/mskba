...
Для первого production smoke superadmin может один раз получить 10 000 ₽ на
bonus-баланс через защищённый bootstrap action. Начисление идемпотентно и
записывается в тот же ledger как `bonus_grant`.
