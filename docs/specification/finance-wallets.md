...
UI позволяет переводить бонусные средства другому подтверждённому пользователю через общий predictive user picker проекта. Кандидаты ищутся по имени, логину и никнейму и подчиняются Identity-настройке `Видимость в поиске` (`DISCOVERABILITY`), включая режим `Определённым пользователям`. Выбранный recipient повторно проверяется на backend, поэтому скрытого пользователя нельзя подставить прямым POST по id. Пополнение реальными деньгами, вывод, перевод real-баланса и покупки пока не подключены. Presentation вызывает Finance use case и не меняет ledger напрямую.
...
между alias одной canonical identity запрещён. Получатель выбирается только из
пользователей, доступных отправителю согласно `DISCOVERABILITY`; эта же проверка
повторяется при выполнении POST, а не только в autocomplete.
...
Финансовая операция коммитится независимо от post-commit доставки уведомления.
Если создание уведомления неожиданно падает, HTTP-ответ всё равно должен явно
сообщить, что перевод уже выполнен, чтобы пользователь не повторил его из-за
ложного 500. Исключение логируется, а UI показывает отдельное предупреждение.
