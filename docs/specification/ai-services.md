...
2. проверяется наличие подтверждённого анфаса и минимум одного профиля;
3. backend читает материализованный total balance пользователя `real + bonus`;
4. при недостаточном балансе AI не вызывается;
5. текущие параметры персонажа, формы, обуви и атрибутов нормализуются в предметный payload;
6. выбранная команда, если есть, повторно авторизуется по активному player membership;
7. payload передаётся в `PlayerCharacterAiGateway`;
8. во время запроса предыдущая 2D/3D-визуализация остаётся на сцене;
9. ошибка AI не очищает существующую модель.
