...
Долговременная запись `VenueOwnership`, связанная с созданным owner membership,
дополнительно хранит независимые `maintenance_commitment_accepted`,
`maintenance_score` и внутренний `maintenance_comment`. Score допускает только
`0, 10, ..., 100`. Эти поля не расширяют договорные permissions и изменяются
только администраторами через отдельный use case; каждое изменение проходит
через общий audit. Публичная интерпретация достоверности описана Task 161 и должна
искать именно действующий owner membership, а не считать `active_marker`
достаточным доказательством действующего договора.
