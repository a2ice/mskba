...
`default_category` — presentation-pattern для однотипных публичных страниц-каталогов MSKBA: площадок, мероприятий, турниров, команд, секций, участников и будущих сущностей с похожей моделью поиска/фильтрации.
...
- иконка этой кнопки по умолчанию `Карточками` и меняется на `Списком` только когда активен `view=list`; переход в `map` не должен оставлять кнопку без иконки;
...
- при переходе `cards/list → map` страница доводит map-result в полезную область viewport; на touch-устройствах это делается без smooth-scroll, чтобы Safari не откладывал фактическую прокрутку до следующего жеста;
- lazy-init карты запускается только после того, как map-result показан и браузер получил минимум два кадра на layout/paint;
...
## Правило viewport для интерактивных карт

Для интерактивных карт MSKBA базовое правило — viewport определяется фактическими отображаемыми точками, а не заранее заданными границами Москвы или другого региона.

- при двух и более различных координатах карта должна охватывать крайние точки с безопасным `zoomMargin`;
- фиксированные региональные bounds допустимы только как явно согласованное продуктовое исключение, но не как default;
- при одной точке или нескольких объектах в одной координате используется осмысленный локальный zoom вместо попытки `fitBounds` в нулевую область;
- после появления ранее скрытого map-контейнера сначала синхронизируется размер viewport (`fitToViewport` или эквивалент), затем применяется fit по данным;
- синхронизация viewport обязательна не только при первом создании карты, но и при каждом повторном показе контейнера после `display:none`/`hidden`; общий Yandex Maps wrapper отслеживает возврат ненулевого размера контейнера через `ResizeObserver` и вызывает `fitToViewport`;
- фильтрация каталога должна автоматически приводить к пересчёту viewport по текущему набору точек после перезагрузки/перерендера данных.

Это правило является общим для интерактивных карт сайта. Текущий `/venues` реализует его в `venue-catalog.js`; остальные существующие Yandex Maps flows уже преимущественно используют bounds своих geo-objects и не должны возвращаться к статическому масштабу без отдельной причины.
...
## Consumer: /participants, /players, /coaches

Каталог участников использует тот же shell без map-mode. Каноническая страница — `/participants`; короткие маршруты `/players` и `/coaches` обслуживаются тем же controller/view и задают предустановленный role filter. Каталог поддерживает cards/list, search по публичному имени/никнейму, role filter на общей странице, mobile filters и стандартную пагинацию.

Sidebar и header-группа используют один порядок: «Игроки», «Тренеры», «Все участники», «Команды». `/teams` остаётся самостоятельным Team-каталогом, но навигационно входит в группу «Участники».


### Приоритет собственного профиля в каталоге участников

Если посетитель авторизован и его canonical user проходит текущую выборку каталога участников (включая preset/filter роли и search), его карточка сортируется первой до пагинации и получает компактную отметку «Мой профиль». Для остальных карточек сохраняется алфавитная сортировка. Privacy-контракт не ослабляется: собственный пользователь проходит viewer-aware настройки через существующий `UserPrivacyAccessService`, но всё равно должен соответствовать статусу аккаунта и активной роли текущей выборки.


### Eligibility каталога участников

`/participants` показывает все подтверждённые canonical accounts, которые не заблокированы/не удалены и разрешены настройкой `DISCOVERABILITY`. Наличие participation role для общей выборки не требуется.

`/players`, `/coaches` и role-filter общей страницы добавляют только одно дополнительное условие — наличие соответствующей **активной** participation role. Приватность детальной страницы роли (`ROLE_PLAYER`, `ROLE_COACH` и т.д.) не исключает пользователя из каталога: она продолжает управлять именно открытием role-page и её блоков.

Каталог не должен обходить приватность содержимого профиля. Если `PROFILE`/distribution consent не разрешают публичные персональные поля, карточка остаётся discoverable, но использует безопасный account identifier (nickname/username), не показывает персональный avatar и выводит состояние «Профиль закрыт» вместо ссылки, которая привела бы к 404.


### Optional parent navigation group

Для связанных каталогов `default-category` поддерживает необязательную секцию `category-pre-navigation`. Она выводится **перед** локальной навигацией как отдельный accordion-item и по умолчанию закрыта; локальная секция текущего каталога остаётся раскрытой. Заголовок задаётся через `sidebarPreNavigationTitle`.

Accordion JS уважает серверное начальное состояние (`is-open` / `aria-expanded` / `hidden`), а не механически раскрывает первый DOM-item. Это позволяет показать контекст родительской группы, не отбирая фокус у текущего каталога.

Текущий consumer: `/teams`. В его sidebar первой закрытой секцией показывается группа «Участники» (`Игроки`, `Тренеры`, `Все участники`, `Команды`), затем раскрытая локальная группа «Команды», затем `Фильтры`.
