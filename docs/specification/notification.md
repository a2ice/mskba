...
Для обычных информационных уведомлений модель может задавать безопасные `action_url` и
`action_text`: CTA отображается и в toast, и в центре уведомлений. Системные ссылки внутри
портала следует сохранять относительными, чтобы они одинаково работали на dev/prod и в разных
доменных окружениях.
